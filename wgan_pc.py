...
def calc_gradient_penalty(netD, real_data, fake_data, device):
    LAMBDA = 10
    alpha = torch.zeros(args["batch_size"]).uniform_(0, 1).to(device)
    ...
    # d(output)/d(inputs)
    # grad_outputs= a sequence of length matching output containing the precomputed
    # gradients w.r.t. each of the outputs

    # return is a tuple so we need [0]
...
if __name__ == '__main__':
    ...
    for epoch in range(args["epoch"]):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            gradient_penalty = calc_gradient_penalty(netD, real_cpu, fake, device=device)
