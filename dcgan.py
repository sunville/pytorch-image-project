...
args = {
    ...
    "img_dir": "/uac/msc/hwu/faces",
    # "data_post_dir": "./post_opc",
    "img_output_dir": "/uac/msc/hwu/fake",
    "checkpoint_dir": "/uac/msc/hwu/checkpoint",
    "img_csv": "/uac/msc/hwu/img_name.csv",
    "channel": 3,
    "latent":100,
    "beta": 0.5,
    "gpu": 1,
...
#            if ((iters % 199 == 0) and epoch % 5 == 0) or ((epoch == args["epoch"] - 1) and (i == len(dataloader)-1)):
            if (epoch == args["epoch"] - 1) and (i == len(dataloader) - 1):
                with torch.no_grad():
    ...
    torch.save(netG.state_dict(), os.path.join(args["checkpoint_dir"], "generator.pkl"))
    torch.save(netD.state_dict(), os.path.join(args["checkpoint_dir"], "discriminator.pkl"))
    ...
    plt.legend()
#    plt.show()
    plt.savefig(os.path.join(args["img_output_dir"], 'g_d_loss.png'))
    ...
    plt.imshow((np.transpose(vutils.make_grid(real_batch.to(device)[:64], padding=5, normalize=True).cpu(),(1,2,0))*255).type(torch.IntTensor))
    plt.subplot(1,2,2)
    plt.axis("off")
    plt.title("Fake Images")
    plt.imshow((np.transpose(img_list[0],(1,2,0))*255).type(torch.IntTensor))
    # plt.show()
    plt.savefig(os.path.join(args["img_output_dir"], 'real_fake.png'))
